...
const category = computed(() => settingsConfig.find(c => c.id === 'account')!)
      ...
      >
        <template v-for="item in section.items" :key="item.key">
          ...
          <SettingsLink
            ...
            :item="item"
          />
          <div v-else-if="item.type === 'action'" class="px-5 py-3.5">
            <button
              class="text-sm font-medium text-primary hover:underline"
              @click="item.key === 'exportData' ? $router.push('/settings/privacy') : null"
            >
              {{ t(item.label) }}
            </button>
